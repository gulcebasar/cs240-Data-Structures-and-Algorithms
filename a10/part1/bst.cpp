...
BinNode * getDelete(ItemType item, BinNode * & parent, BinNode * root);
...
{
    return height2(this->root)-1; // calls the helper function (-1 for the root)
}
...
void display2(BinNode * root, ostream & out)
{
    if( root != NULL )
    {
        ...
        out << root->item << endl; // displays root
...
BinNode * getDelete(ItemType item, BinNode * & parent, BinNode * root) // function to get parent and the node with item
{
